 ...
 * Title: Lab4 UDP Peer2
 * Description: This program runs the udp peer2 for the peer2peer file distribution.
 */
#include <stdio.h>
#include <stdlib.h>
...
int main() {
    while (1) {
        char input;
		printf("This is peer 2: Type S to send or R to receive, and Q to quit\n");
        scanf("%c", &input);
        if (input == 'S') {
            printf("Peer 2 sending!\n");
            printf("Please enter the filename you want to send: ");
            scanf("%s", fsrc);
            server();
        } else if (input == 'R') {
            printf("Peer 2 receiving\n");
